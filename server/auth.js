...
function login(req, res, next) {
    ...
    db.query('SELECT id, firstName, lastName, email, id as externalUserId, password__c AS password FROM salesforce.contact WHERE email=$1', [creds.email], true)
        .then(function (user) {
            if (!user) {
                return res.send(401, invalidCredentials);
            }
...
function createUser(user, password) {
    ...
    db.query('INSERT INTO salesforce.contact (email, password__c, firstname, lastname, leadsource, accountid) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, firstName, lastName, email, id as externalUserId',
        [user.email, password, user.firstName, user.lastName, 'Simple Sessions App', config.contactsAccountId], true)
